                ...
                <div class="flex-1 min-w-0">
                    ...
                    @if($notification->refund_data)
                        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mt-2">
                            <p class="text-xs font-semibold text-yellow-800 mb-1">💳 Detail Rekening:</p>
                            <p class="text-xs text-yellow-900">{{ $notification->refund_data['bank_no'] }} a/n {{ $notification->refund_data['owner_name'] }}</p>
                            @if($notification->refund_data['proof_url'])
                                <a href="{{ $notification->refund_data['proof_url'] }}" target="_blank" class="text-xs underline mt-1 inline-block">📸 Lihat Bukti</a>
                            @endif
                        </div>
                    @endif
...
@if($notification->refund_data)
    ...
    @if($notification->order && $notification->order->payment_status !== 'refunded')
        ...
        <form
            action="{{ route('admin.notifications.process-refund', $notification->id) }}"
            method="POST">
            ...
            <button
                ...
                class="px-3 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-xl transition">
    ...
    @else
        ...
        <div class="px-3 py-2 bg-green-100 text-green-700 text-xs font-bold rounded-xl">
            ✅ Sudah Refund
        </div>
